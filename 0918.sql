...
-- 비등가 조인 : 동일 컬럼(열, 레코드) 없이 다른 조건을 사용하여 조인할때 사용(일반적인 경우는 아님)
SELECT * FROM EMP;
...
-- 자체조인 : SELF 조인이라고도 함. 같은 테이블을 두번 사용하여자체 조인
-- EMP 테이블에서 직속상관이 사원번호는 MGR에 있음
...
-- 외부 조인 : 동등 조인의 경우 한쪽 컬럼이 없으면 해당행을 표시되지않음
-- 외부 조인은 내부 조인과 다르게 다른 한쪽의 값이 없어도 출력 DELETE
-- 외부 조인은 동등 조인 조건을 만족하지 못해 누락되는 행을 출력하기 위해서                        
...
-- NATURAL JOIN : 등가ㅑ조인대신 사용, 자동으로 같은열을 찾아 조인해줌
...
FROM EMP E JOIN DEPT D USING (DEPTNO)
GROUP BY DEPTNO;
-- 3. 모든 부서 정보와 사원 정보를 부서번호, 사원이름순으로 정렬
...
SQL 문 안에 작성하는 작은 SELECT 문을 의미합니다.
킹이라는 이름을 가진 사원의 부서 이름을 찾기위함*/
...
-- EMP테이블에서 사원정보중에서 사원이름이 'ALLEN 인 사원의 추가수당보다 많은 추가수당을 받는 사원 출력
SELECT *
FROM EMP
...
사원 정보와 소속부서 정보를 조회하는 경우에 대한 쿼리를 작성*/
...
-- ANY : 메인 쿼리의 비교 조건이 서브 퀄리의 여러 검색결과 중 하나 이상 만족되면 반환
...
-- 30번 부서 사원들의 급여보다 적은 급여를 받는 사원정보 출력
-- ALL : 모든 조건을 만족하는 경우 성립
SELECT *
FROM EMP
...
/* 다중열 서브쿼리 : 서브 쿼리의 결과가 두 개 이상의 컬럼으로 변환되어 메인 쿼리에 전달하는 쿼리 */
