...
SELECT * FROM EMP;
-- 1. 전체 사원 중 ALLEN과 같은 직책(JOB)인 사원들의 사원정보, 부서정보를 다음과 같이 출력하는 SQL문을 작성
-- 조인과 서브쿼리 필요
SELECT ENAME, DEPTNO
FROM EMP
...
-- 2. 전체 사원의 평균 급여(SAL)보다 높은 급여를 받는 사원들의 사원정보, 부서정보, 급여등급정보를 출력하는 SQL문을
-- 작성하세요 (단 출력할 때 급여가 많은 순으로 정렬하되 급여가 같을 경우에는 사원 번호를 기준으로 오름차순 정렬)
...
-- 3. 10번 부서에 근무하는 사원 중 30번 부서에는 존재하지 않는 직책을 가진 사원들의 사원정보, 부서정보를 다음과 같이
-- 출력하는 SQL문을 작성하세요.
SELECT E.EMPNO, E.ENAME, E.JOB, E.DEPTNO, D.DNAME, D.LOC
 ...
 WHERE E.DEPTNO = D.DEPTNO
   ...
   AND JOB NOT IN (SELECT DISTINCT JOB
                     FROM EMP
                    WHERE DEPTNO = 30);
/* 4. 직책이 SALESMAN인 사람들의 최고 급여보다 높은 급여를 받는 사원들의 사원정보, 급여등급정보를 다음과 같이 출력하는
SQL문을 작성하세요(단 서브쿼리를 활용할 때 다중행 함수를 사용하는 방법과 사용하지 않는 방법을 통해 사원번호를 기준으로
오름차순 정렬하세요.)*/
/* [다중행 사용하지 않는 방법]*/
...
-- DML을하기위해서 임시 테이블 생성
-- 기존의 DEPT TABLE 을 복사해서 DEPT_TEMP 테이블 생성
...
AS SELECT * FROM EMP;
WHERE 1 != 1; -- 테이블을 복사해 새로운 테이블을 만들 때 데이터는 복사하고싶지 않을 때 사용
...
-- 서브쿼리를 이용한 INSERT
INSERT INTO EMP_TEMP(EMPNO, ENAME, JOB, KGR, HIREDATE, SAL, COMM, DEPTNO )
...
-- 테이블에 있늗 데이터 삭제하기
CREATE TABEL EMP_TEMP2
AS SLELCT* FROM EMP;

SELELCT * FROM EMP_TMEP2
