...
-- SQL문의 마지막 절은 세미콜론(;)으로 끝난다.
-- 사원번호와 부서번호만 나오도록 EMP 테이블 조회하기.
SELECT EMPNO, DEPTNO FROM EMP;
-- 별칭 설정하기 : AS, 열이름 또는 컬럼이름을 별칭으로 표시할 수 있음
SELECT ENAME AS 이름, SAL AS 급여, SAL*12+COMM AS 연봉, COMM AS 성과급
FROM EMP;

-- 중복 제거하기 : DISTINCT, 데이터를 조회할 때 값이 중복되는 행이 여러 개 조회되는 경우, 값이 중복된 행을 한개씩만 선택하고자 하는 경우 사용
...
SELECT *
FROM EMP
WHERE SAL >= 2000 AND SAL <= 3000; -- 급여가 2000~3000인 사람

SELECT *
FROM EMP
WHERE SAL BETWEEN 2000 AND 3000; -- 급여가 2000~3000인 사람

SELECT *
FROM EMP
WHERE SAL NOT BETWEEN 2000 AND 3000; -- 급여가 2000~3000이 아닌사람

-- 사원번호가 7689 ~ 9702 사이인 사원 조회
SELECT *
FROM EMP
...
-- LIKE, NOT LIKE 연산자 : 일부 문자열이 포함되어있는지 여부를 확인하는 연산자, 주로 검색에서 사용
-- % : 길이와 상관없이 모든 문자를 의미
-- _ : 문자 1자를 의미
SELECT EMPNO, ENAME
FROM EMP
WHERE ENAME LIKE '%K%'; -- ENAME 에 K 가 포함된 모든것

SELECT *
FROM EMP
WHERE ENAME LIKE '_L%'; -- ENAME 중에 L 앞에 문자가 하나만 있는것 뒤에는 상관없음

-- 사원 이름에 AM이 포함되어있는 사원데이터 출력
-- 사원이름에 AM이 포함되어있지않은 사원데이터 출력
